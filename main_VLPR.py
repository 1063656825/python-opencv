...
class Surface(ttk.Frame):
    ...
    def show_roi1(self, r1, roi1, color1):
        if r1:
            roi1 = cv2.cvtColor(roi1, cv2.COLOR_BGR2RGB)
            roi1 = Image.fromarray(roi1)
            self.imgtk_roi1 = ImageTk.PhotoImage(image=roi1)

            self.roi_ctl.configure(image=self.imgtk_roi1, state='enable')
            self.r_ctl.configure(text=str(r1))
            self.update_time = time.time()
            try:
                c = self.color_transform[color1]
    ...
    def from_pic(self):
        ...
        if self.pic_path:
            ...
            # r_color颜色识别车牌信息  roi_color 颜色图片  color——color车牌颜色
    @staticmethod
    # 静态方法类或实例均可调用
